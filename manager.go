...
// RegisterNotifier registers a notifier that will execute all reloaders when
...
// RegisterReloader registers a reloader to the manager.
//
...
func (m *Manager) RegisterReloader(priority int, r Reloader) {
	...
	m.reloaderGroupByPriority[priority] = rg
}
...
func (m *Manager) reloadGroups(ctx context.Context, id string) error {
	if len(m.reloaderGroupByPriority) == 0 {
		return nil
	}
	...
	// Sort groups.
	reloderGroups := make([]reloaderGroup, 0, len(m.reloaderGroupByPriority))
	for _, rg := range m.reloaderGroupByPriority {
